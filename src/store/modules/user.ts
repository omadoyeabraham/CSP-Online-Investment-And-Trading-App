...
const getters = {
  ...
  getAccountManagerEmail: state => {
    return state.accountManagerEmail
...
const mutations = {
  ...
  [mutationTypes.SAVE_AUTHENTICATED_USER_TO_STORE](state, userData) {
    ...
    state.firstName = userData.customer
      ? `${userData.customer.firstName}`.toLowerCase()
      : state.firstName;
    state.lastName = userData.customer
      ? `${userData.customer.lastName}`.toLowerCase()
      : state.lastName;
    state.portalUsername = userData.customer
      ? `${userData.customer.portalUsername}`.toLowerCase()
      : state.portalUsername;
