...
let getTradeOrders = (cacheStatus: number) => {
  ...
  axios({
    ...
    if (response.data.item !== undefined) {
      ...
      tradeOrders =
        tradeOrders.constructor === Array ? tradeOrders : [tradeOrders];
...
let setSecurityMarketSnapShot = snapShotData => {
  ...
  // Calculating bids
  let bidLevels =
    marketSnapShot && marketSnapShot.bidLevels ? marketSnapShot.bidLevels : [];
  let bids = [];
  let bidsTotal = 0;
  ...
  let trades =
    marketSnapShot && marketSnapShot.trades ? marketSnapShot.trades : [];
  let priceMovements = [];
  ...
  store.commit(
    mutationTypes.SAVE_SELECTED_SECURITY_MARKET_SNAPSHOT,
    currentMarketData
  );
};
...
let getSecurityStatusInfo = (securityName: any) => {
  axios({
    ...
    .then(response => {
      ...
      store.commit(
        mutationTypes.SAVE_SELECTED_SECURITY_STATUS_INFO,
        statusInfo
      );
    })
    .catch(error => {
